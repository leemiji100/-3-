...
🌟 왜 이 부분이 중요한가요?
과제 요구사항 중에 **"도구 1개를 선정하고 선정 이유를 작성한다"**라는 항목이 있는데, 학생님은 단순히 "유명해서"라고 하지 않고:
비용 측면 (무료 플랜에서 Router 제공)
기능 측면 (데이터 가공의 강력함)
이 두 가지를 명확함
...
🛠 [프로젝트 2] 워크플로우 단계별 상세 설명 (보충)
1단계: 기초 데이터 구축 (Google Sheets)
설명: 자동화의 '재료'가 되는 데이터를 준비하는 단계입니다.

핵심 내용:
구글 시트에 날짜, 교시, 과목명, 이수구분(전공/교양), 강의실 컬럼을 생성합니다.
보충 포인트: "데이터의 정형화가 중요합니다. 특히 '이수구분' 열은 이후 Router에서 필터링 조건으로 사용되기 때문에 오타 없이 입력되어야 합니다."
2단계: 스케줄 트리거 및 데이터 검색 (Make - Search Rows)
설명: 정해진 시간에 데이터를 찾아오는 '시작' 단계입니다.
핵심 내용:

Schedule: 매일 아침 8시에 시나리오가 실행되도록 설정합니다.
Search Rows: 구글 시트에서 날짜 컬럼이 Today(오늘)와 일치하는 행만 필터링해서 가져옵니다.
보충 포인트: "불필요한 데이터 처리를 줄이기 위해, 시트 전체를 가져오는 것이 아니라 '오늘 날짜'에 해당하는 데이터만 선별적으로 추출하여 효율성을 높였습니다."
3단계: 루터(Router)를 활용한 조건 분기
설명: 가져온 데이터를 성격에 따라 '분류'하는 핵심 지능 단계입니다.

핵심 내용:
Router 설정: 하나의 경로를 두 개(전공/교양)로 나눕니다.
Filter 설정:
Path A (전공): '이수구분'이 '전공'인 경우만 통과.
Path B (교양): '이수구분'이 '교양'인 경우만 통과.
보충 포인트: "Make의 Router 기능을 통해 단일 워크플로우 내에서 다중 로직 처리가 가능해졌습니다. 이는 사용자에게 과목 성격에 맞는 맞춤형 알림 메시지를 구성할 수 있는 기반이 됩니다."

4단계: 최종 메시지 전송 (Discord/Slack)
설명: 가공된 데이터를 사용자에게 '전달'하는 마지막 단계입니다.
핵심 내용:
각 경로 끝에 Discord 또는 Slack 모듈을 연결합니다.
데이터 매핑: 시트에서 가져온 과목명, 강의실 데이터를 메시지 본문에 변수로 삽입합니다.
보충 포인트: "전공 수업은 '🔥전공 집중!'이라는 문구를 추가하고, 교양 수업은 '🌱교양 수업'이라는 문구를 추가하여 메시지의 가독성과 직관성을 높였습니다."
